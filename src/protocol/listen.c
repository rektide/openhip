...
#include "/usr/src/linux/include/linux/pfkeyv2.h"               /* PF_KEY_V2
                                                                 *support     */
#include "hip.h"
                  ...
                  while (buffi < err)
                    {
                      ...
                      switch (pfkey_ext->sadb_ext_type)
                        ...
                        case SADB_EXT_ADDRESS_SRC:
                          l += print_address(&obuff[l],
                                             (struct sadb_address*)pfkey_ext);
                          printf(",address_S");
                          break;
...
int sadb_verify(char *data, int num)
{
  ...
  while (num)
    {
      if (ext->sadb_ext_type == 0)
        {
          break;
        }
      printf("(%d)", ext->sadb_ext_type);
      ext =
        (struct sadb_ext *)((char *)ext +
                            (ext->sadb_ext_len * IPSEC_PFKEYv2_ALIGN));
      num--;
    }
  printf("\n");
  return(0);
...
int print_address(char* buff, struct sadb_address* addr)
{
  ...
  sprintf(
    s,
    "(len=%d exttype=%d proto=%d prefixlen=%d reserved=%d fam=%d addr=%s)\n",
...
int print_sa(char* buff, struct sadb_sa* sa)
{
  ...
  sprintf(
    s,
    "(len=%d exttype=%d SA=%08x replay=%d state=%d auth=%d encrypt=%d flags=%d)\n",
    sa->sadb_sa_len,
    sa->sadb_sa_exttype,
    sa->sadb_sa_spi,
    sa->sadb_sa_replay,
